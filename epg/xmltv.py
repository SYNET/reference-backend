...
def elem_to_programme(elem):
    ...
    if elem.get('start') is None or elem.get('stop') is None:
        # start and stop attributes are mandatory
        return None
    ...
    append_text(d, 'catalog_id', elem, False)
    append_text(d, 'genre', elem, False)
...
def read_programmes(fp=None, tree=None):
    ...
    if fp:
        et = ElementTree()
        tree = et.parse(fp)
    retval = [elem_to_programme(elem) for elem in tree.findall('programme')]
    try:
        while True:
            retval.remove(None)
    except:
